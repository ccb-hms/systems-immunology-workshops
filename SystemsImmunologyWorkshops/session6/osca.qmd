...
```{r pca2}
pca2 <- reducedDim(sce, "PCA")[,1:2]
head(pca2)
reducedDim(sce, "PCA2") <- pca2
sce
head(reducedDim(sce, "PCA2"))
```
:::
...
```{r qc}
df <- data.frame(qcstats[,c("sum", "subsets_Mito_percent")], 
...
```{r qc-plot}
ggplot(df, aes(x = sum, y = subsets_Mito_percent)) + 
    geom_point(aes(color = discard), size = 0.5) + theme_bw()
```
...
df <- data.frame(reducedDim(sce.sub, "PCA")[,1:2], cluster = sce.sub$cluster)
head(df)
```
...
ggplot(df, aes(x = PC1, y = PC2)) + 
    geom_point(aes(color = cluster), size = 0.5) + theme_bw()
```

Alternatively, use the the `plotReducedDim` function from the 
[scater](https://bioconductor.org/packages/scater) package for convenience.

```{r pca-plot-scater}
plotReducedDim(sce.sub, "PCA", color_by = "cluster")
```
...
sce.sub$cluster <- factor(sce.sub$cluster, levels = 1:10)
plotReducedDim(sce.sub, "TSNE", colour_by = "cluster")
```
...
plotReducedDim(sce.sub, "UMAP", colour_by = "cluster")
```
...
::: {.callout-tip icon="false" collapse="true"}
## Solution
Parameterize the clustering:

```{r cluster-par}
cargs <- list(objective_function = "modularity", 
              resolution_parameter = 0.5)
blpar <- NNGraphParam(cluster.fun = "leiden", cluster.args = cargs)
```

Cluster:

```{r cluster}
nn <- clusterCells(sce, use.dimred = "PCA", BLUSPARAM = blpar)
table(nn)
```
...
```{r cluster-viz}
sce$leiden <- nn
plotReducedDim(sce, "UMAP", color_by = "leiden")
```
...
plotExpression(sce, features = top.markers, x = "label", colour_by = "label")
```
