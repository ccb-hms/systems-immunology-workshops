...
```{r nr-features}
nr.features <- round(0.01 * nrow(pbmc.data))
nr.features
```

```{r create-pbmc2}
# Initialize the Seurat object with the raw (non-normalized data).
pbmc2 <- CreateSeuratObject(counts = pbmc.data, project = "pbmc", 
                            min.cells = nr.cells, min.features = nr.features)
pbmc2
```
...
```{r filter-thresh}
thresh.data <- pbmc.data[rm >= 0.01, cm >= 0.01]
dim(thresh.data)
```

Create Seurat object on filtered count matrix:

```{r create-pbmc-thresh}
pbmc3 <- CreateSeuratObject(counts = thresh.data, project = "pbmc")
pbmc3
```
:::
...
Note: `FeatureScatter` is typically used to visualize feature-feature relationships, 
but can be used for other numeric data stored in the Seurat object, incl. cell 
metadata column, PC scores, among others.
:::
...
These seem to represent low abundant cell types.
:::
...
The base R way of doing things:

```{r}
spl <- split(pbmc.markers, pbmc.markers$cluster)
getTop2FC <- function(df) 
{
    ind <- order(df$avg_log2FC, decreasing = TRUE)
    df <- df[ind,]
    df <- head(df, n = 2)
    return(df)
} 
spl <- lapply(spl, getTop2FC)
do.call(rbind, spl)
```

The tidyverse way of doing things:
...
In this case, we are plotting the top 10 markers 
(or all markers if less than 10) for each cluster.
...
```{r sobj2sce-solve}
p1 <- plotExpression(pbmc.sce, features = "MS4A1", x = "ident") + 
...
```{r sobj2sce-solve-ext, fig.width = 10}
...
```{r sce2sobj-solve}
p1 <- DimPlot(manno.seurat, reduction = "PCA", group.by = "Timepoint") + NoLegend()
p1
```
:::
...
::: {.callout-tip icon="false" collapse="true"}
## Solution
```{r sce2sobj-solve-ext, fig.width = 10}
p2 <- RidgePlot(manno.seurat, features = "ACTB", group.by = "Timepoint")
p1 + p2
```
