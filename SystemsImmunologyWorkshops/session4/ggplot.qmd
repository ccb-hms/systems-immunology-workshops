...
For this lesson, you will need the `new_metadata` data frame. 
Load it into your environment as follows:
...
## Advanced
When publishing, it is helpful to ensure all plots have similar formatting. 
To do this we can create a custom function with our preferences for the theme.
Create a function called `personal_theme` which takes no arguments and
...
First try making a simply interactive scatterplot with `new_metadata` and the same axes as the ggplot scatterplot. 
If you are able to do so, try adding [dropdown menus](https://plotly.com/r/dropdowns/) which allow you to choose which column of `new_metadata` to color the points by. 
:::
