...
1.  Subset the `SummarizedExperiment` object `se` created above to genes (rows) with at least 8 reads mapped across all samples. Hint: use `rowSums`.
...
4.  Carry out a *t*-test for each of the selected 100 genes. Test for differences in mean read count per million reads mapped between the dexamethasone treated and untreated sample group. Annotate the resulting *p*-values as a new column in the `rowData` slot of your `SummarizedExperiment`.\
