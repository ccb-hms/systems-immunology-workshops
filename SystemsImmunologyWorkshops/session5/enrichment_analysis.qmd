...
**Task:** Carry out an over-representation test in the context of the `airway` dataset, where we found 3,994 differentially expressed genes (out of a total of 26,037 genes tested). Let's assume again that 28 DE genes overlap with a gene set of interest, which contains in total 170 genes.

```{r}
deTable <-
     matrix(c(28, 142, 3994 - 28, 26037 - 170 - 3994),
            nrow = 2,
            dimnames = list(c("DE", "Not.DE"),
                            c("In.gene.set", "Not.in.gene.set")))
deTable
...
We load the [airway](http://bioconductor.org/packages/airway) dataset
...
nrDE \<- function(se, de.method) { message(de.method) se \<- deAna(se, de.method = de.method) nr.de \<- sum(rowData(se)\$ADJ.PVAL \< 0.05) return(nr.de) }

nrDE(airSE, "limma") de.methods \<- c("limma", "edgeR", "DESeq2") sapply(de.methods, nrDE, se = airSE)
...
Note #2: Gene set libraries from the comprehensive [Enrichr](https://amp.pharm.mssm.edu/Enrichr/#stats) collection for 5 different species can be obtained using `getGenesets` with `db = "enrichr"`. For example, gene sets based on transcription factor interactions can be obtained from Enrichr via:
...
As outlined in the first section, ORA tests the overlap between DE genes (typically DE *p*-value \< 0.05) and genes in a gene set based on the hypergeometric distribution.
...
*Exercise*: Carry out a GO overrepresentation analysis for the `airSE` based on the molecular function (MF) ontology. How many significant gene sets do you observe in each case?
...
```{r, echo=FALSE, fig.width = 3}
knitr::include_graphics("gsea.png")
```
...
```{R}
roast.air <- sbea(method = "roast", se = airSE, gs = go.gs)
gsRanking(roast.air)  
```
...
*Homework:* Carry out `SPIA` and `GGEA` for the `airSE` and combine the results. How many gene sets are rendered significant by both methods?
